...
public:
  ...
  virtual std::iostream &GetStream() override { return backingStream; }
};

HostVolume::HostVolume(short vRefNum) : Volume(vRefNum) {
...
long HostVolume::GetDirectoryID(const fs::path &dirPath) {
  if (fs::exists(dirPath) && !fs::is_directory(dirPath)) {
    std::cerr << "Warning: GetDirectoryID should only be used on directories! "
              << dirPath << "\n";
  }
...
FSSpec HostVolume::ToFSSpec(const fs::path &fullPath) {
  ...
  snprintf((char *)spec.cName, 256, "%s",
           (const char *)fullPath.filename().u8string().c_str());
  return spec;
}
...
OSErr HostVolume::OpenFork(const FSSpec *spec, ForkType forkType,
                           char permission,
  ...
  auto path = ToPath(spec->parID, (char *)spec->cName);
  ...
  if (forkType == DataFork) {
    ...
    handle =
        std::make_unique<HostForkHandle>(DataFork, permission, path, *spec);
  } else {
    // We want to open a resource fork on the host volume. It is likely stored
    // as <NAME>.rsrc.
    path += ".rsrc";
    if (!fs::is_regular_file(path)) {
      return fnfErr;
    ...
    handle =
        std::make_unique<HostForkHandle>(ResourceFork, permission, path, *spec);
...
static bool CaseInsensitiveAppendToPath(fs::path &path, const u8string &element,
  ...
  for (const auto &candidate : fs::directory_iterator(path)) {
    ...
    if (candidate.is_regular_file() &&
        candidateFilename.extension() == ".rsrc") {
      candidateFilename.replace_extension("");
    }

    // Convert candidate filename to uppercase for case-insensitive comparison
    const u8string uppercaseCandidateFilename =
        UppercaseCopy(candidateFilename.u8string());
...
OSErr HostVolume::FSMakeFSSpec(long dirID, const u8string &fileName,
                               FSSpec *spec) {
  if (dirID < 0 || (unsigned long)dirID >= directories.size()) {
    throw std::runtime_error(
        "HostVolume::FSMakeFSSpec: directory ID not registered.");
  }
...
OSErr HostVolume::DirCreate(long parentDirID, const u8string &directoryName,
                            long *createdDirID) {
  ...
  if (fs::exists(path)) {
    if (fs::is_directory(path)) {
      ...
      std::cerr << __func__
                << ": a file with the same name already exists: " << path
                << "\n";
      return bdNamErr;
    }
  }
...
OSErr HostVolume::FSpCreate(const FSSpec *spec, OSType creator, OSType fileType,
                            ScriptCode scriptTag) {
  (void)creator;
  (void)fileType;
  (void)scriptTag;

  std::ofstream df(ToPath(spec->parID, (char *)spec->cName));
  df.close();
