...
typedef struct FSSpec {
  // Volume reference number of the volume containing the specified file or
  // directory.
  short vRefNum;

  // Parent directory ID of the specified file or directory (the directory ID of
  // the directory containing the given file or directory).
  long parID;

  // The name of the specified file or directory. In Carbon, this name must be a
  // leaf name; the name cannot contain a semicolon. WARNING: this is a C string
  // encoded as UTF-8, NOT a pascal string encoded as MacRoman! Mac application
  // code expecting "name" (the pascal string) must be adjusted.
  Str255 cName;
} FSSpec;
...
typedef Handle GDHandle; // GDevice handle. Game code doesn't care about GDevice
                         // internals, so we just alias a generic Handle
...
typedef struct SCStatus {
  UnsignedFixed scStartTime; // starting time for play from disk (based on audio
                             // selection record)
  UnsignedFixed scEndTime;   // ending time for play from disk (based on audio
                             // selection record)
...
// for pomme implementation purposes we don't care about the 68000/ppc specifics
// of universal procedure pointers
typedef SndCallBackProcPtr SndCallbackUPP;

typedef struct SndChannel {
  SndChannelPtr nextChan;
  SndCallBackProcPtr callBack;
  long long userInfo; // free for application's use (Pomme: made it 64 bit so
                      // app can store ptrs)
