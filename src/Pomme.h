...
OSErr FSMakeFSSpec(short vRefNum, long dirID, const char *cstrFileName,
                   FSSpec *spec);
...
// Open a file's data fork with fsRdPerm (TODO: it should be fsCurPerm, but we
// don't support fsCurPerm yet) in directory 0 of the given volume. It is legal
// to pass in a colon-separated hierarchical path in cName. (Note: this is an
// ancient function that predates HFS - IM vol II, 1985)
OSErr FSOpen(const char *cName, short vRefNum, short *refNum);

// Open a file's resource fork with fsRdPerm (TODO: it should be fsCurPerm, but
// we don't support fsCurPerm yet) in directory 0 of the application's volume.
// It is legal to pass in a colon-separated hierarchical path in cName. (Note:
// this is an ancient function that predates HFS - IM vol II, 1985)
...
OSErr ResolveAlias(const FSSpec *spec, AliasHandle alias, FSSpec *target,
                   Boolean *wasChanged);

OSErr FindFolder(short vRefNum, OSType folderType, Boolean createFolder,
                 short *foundVRefNum, long *foundDirID);

OSErr DirCreate(short vRefNum, long parentDirID, const char *cstrDirectoryName,
                long *createdDirID);
...
void AddResource(Handle theData, ResType theType, short theID,
                 const char *name);
...
void CopyBits(const PixMap *srcBits, PixMap *dstBits, const Rect *srcRect,
...
PaletteHandle NewPalette(short entries, CTabHandle srcColors, short srcUsage,
                         short srcTolerance);
...
// No-op in Pomme.
// TODO: do something about `grow` and return a large integer to make it look
// like we have tons of memory.
static inline Size MaxMem(Size *grow) {
  (void)grow;
  return 0;
}
...
static inline Boolean LockPixels(PixMapHandle handle) {
  (void)handle;
  return true;
} // no-op; shall always return true

// No-op in Pomme.
// If the Memory Manager started up in 24-bit mode, strips flag bits from 24-bit
// memory addresses; otherwise (in 32-bit mode), returns the address unchanged.
...
// Allocates a handle of the given size and copies the contents of srcPtr into
// it
...
// Number of ticks elapsed since system startup (1 tick = approx. 1/60 of a
// second)
UInt32 TickCount();
...
// Pomme extension
Boolean Pomme_DecompressSoundResource(SndListHandle *sndHandlePtr,
                                      long *offsetToHeader);
